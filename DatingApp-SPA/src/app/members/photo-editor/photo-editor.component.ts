...
export class PhotoEditorComponent implements OnInit {
  @Input('photosFromParent') photos: Photo[];
  ...
  private _initializeUploader() {
    this.uploader = new FileUploader({
      url: `${this._baseUrl}/users/${this.authService.decodedToken.nameid}/photos`,
    ...
    this.uploader.onSuccessItem = (item, response, status, headers) => {
      if (response) {
        ...
        if (photo.isMain) {
          // will update the service photo to be updated in other places
  ...
  /**
   * Button when pressed will set a new main photo
   */
  setMainPhoto(photo: Photo): void {
    this.userService.setMainPhoto(this.authService.decodedToken.nameid, photo.id)
      .subscribe(() => {
        ...
        // swap theobject properties of old main photo and new main photo
